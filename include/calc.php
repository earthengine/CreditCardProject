    ...
    function sort_cards($data, $cardinfos){
    	...
    	foreach ($cardinfos as &$cardinfo){
			...
			foreach ($earnings as &$earning){
				$cid = $earning["compare_id"];
				if(array_key_exists($cid, $card_earnings)){
					$twe = $earning["total_wallet_earnings"];
					if($card_earnings[$cid] >= $earning[$twe]) 
						continue;
				}
 		...
 		foreach ($data->cards as &$_cardspend){
			...
			$total_points = 0;
 			foreach ($cardinfo->tiers as &$tier){
 				if(is_null($tier->visa_points_per_dollar))
 					break;			
				...
				$cap_amount = $tier->points_cap / $avg_points_per_dollar;
 				if(is_null($tier->points_cap) ||
	 						$amount_rest<=$cap_amount){
 			...
 			$result[$idx] = array("wallet_id" => $cardspend->id,
...
/*
{"cards":[{"id":11,"monthly_spend":4000,"balance_percent":100,"amex_percent":20}],"answers":[1,2,1,1,1,1]}
*/
